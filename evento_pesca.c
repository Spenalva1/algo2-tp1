...
arrecife_t* crear_arrecife(const char* ruta_archivo){
  ...
  while (leidos == CANT_A_LEER && !error){
    ...
    nuevo_vector_pokemon = realloc((*arrecife).pokemon, sizeof(pokemon_t) * (*arrecife).cantidad_pokemon);
...
int trasladar_pokemon(arrecife_t* arrecife, acuario_t* acuario, bool (*seleccionar_pokemon) (pokemon_t*), int cant_seleccion){
  ...
  while(i < arrecife->cantidad_pokemon && !error && encontrados < cant_seleccion){
    if((*seleccionar_pokemon)((arrecife->pokemon)+i)){
      ...
      nuevo_vector_pokemon = realloc((*acuario).pokemon, sizeof(pokemon_t)*((*acuario).cantidad_pokemon));
          
      if(nuevo_vector_pokemon == NULL)
        error = true;

      (*acuario).pokemon = nuevo_vector_pokemon;
      (*acuario).pokemon[(*acuario).cantidad_pokemon - 1] = *(arrecife->pokemon+i);

      //SACARLO DEL ARRECIFE
